(function registerExamsPage(global) {
  ...
  function renderRangeForm(year, questionCount) {
    const start = dom.createElement("input", { className: "text-input", attributes: { type: "number", min: "1", max: String(questionCount), value: "1", "aria-label": "شروع بازه" } });
    const end = dom.createElement("input", { className: "text-input", attributes: { type: "number", min: "1", max: String(questionCount), value: String(questionCount), "aria-label": "پایان بازه" } });
