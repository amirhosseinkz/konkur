(function registerLastNightPage(global) {
  ...
  function randomDifficult(subjectId) {
    ...
    const item = difficult[Math.floor(Math.random() * difficult.length)] || app.features.learning.randomItems("flashcards", { subjectId, limit: 1 })[0];
    if (item) global.location.hash = item.route;
    app.ui.notifications.announce(item ? item.title : "فلش‌کارت پیدا نشد.", item ? "info" : "warning");
  }
  function randomTrap(subjectId) {
    const item = app.features.learning.randomItems("exam-traps", { subjectId, limit: 1 })[0];
    if (item) global.location.hash = item.route;
    app.ui.notifications.announce(item ? item.title : "دام آزمون پیدا نشد.", item ? "info" : "warning");
  }
  ...
  function subjectBlock(subject) {
    ...
    const randomDifficultButton = dom.createElement("button", { className: "button", text: "فلش‌کارت تصادفی", attributes: { type: "button" } });
    randomDifficultButton.addEventListener("click", () => randomDifficult(subject.id));
    const randomTrapButton = dom.createElement("button", { className: "button", text: "دام آزمون تصادفی", attributes: { type: "button" } });
    randomTrapButton.addEventListener("click", () => randomTrap(subject.id));
    const detailsEl = dom.createElement("details", expanded ? { attributes: { open: "true" } } : {}, [
      ...
      dom.createElement("div", { className: "empty-actions" }, [markButton, randomRuleButton, randomDifficultButton, randomTrapButton])
    ]);
