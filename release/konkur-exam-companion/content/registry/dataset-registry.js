  ...
  class DatasetRegistry {
    ...
    getSearchDocuments() {
      ...
      return active.flatMap((entry) => {
        ...
        const title = entry.datasetType === "questions" && record.year && record.questionNumber
          ? `سؤال ${record.questionNumber} آزمون ${record.year}`
          : this.localizedText(record.title || record.front || record.statement || record.trap || record.text);
        if (!title) {
          return [];
        }
        return [{
          ...
          title,
          body: entry.datasetType === "questions" ? `${record.id} ${this.collectSearchBody(record)}` : this.collectSearchBody(record),
          route: this.routeFor(entry.datasetType, record),
    ...
    routeFor(datasetType, record) {
      if (datasetType === "questions" && record && record.year && record.questionNumber) {
        return `#/exams/${record.year}/practice/custom?start=${record.questionNumber}&end=${record.questionNumber}`;
      }
      const routeByType = {
        subjects: "#/subjects",
        topics: "#/subjects",
