...
Version: 1, with Source Record schema version 2
...
|---|---|
| Source Record | Version 1: `source:<year-or-reference>:<source-type>:<stable-slug>`. Version 2 records preserve stable IDs but carry explicit family, group or set, and paper fields. |
...
| `schemaVersion` | positive integer | Supported values are `1` for legacy Computer Engineering records and `2` for multi-family source records. |
...
Version 2 Source Records also require `examFamilyId`, `examGroupId`, `examSetCode`, and `paperCode`. `examFamilyId` is `national-entrance` or `masters-entrance` for an exam artifact and null for a `verified-reference`. National Entrance papers, keys, and corrections require one canonical group ID and null `examSetCode`; Master's Entrance papers, keys, and corrections require one official annual `examSetCode` and null `examGroupId`. `catalog-booklet` may use null group and set values when it applies to the entire exam family. `paperCode` is a lowercase ASCII publisher identifier or null.

Optional fields: `publicationDate` as ISO date, `edition` as string, and `language` as BCP 47 tag.

Validation: `localFilename` and `checksum` must either both be present or both be null. A local file is not considered acquired until its checksum is recorded. An `official-correction` identifies its target through `paperSourceId` and, when applicable, `answerKeySourceId`. Every `correctionSourceId` resolves to an official-correction Source Record targeting this paper or key. Version 2 keys and corrections must match their paper's exam family and group or set.
